...
export default function GameScreen({ profile, resumeData, onSaveGame, onEnd }) {
  ...
  function handleChoice(action) {
    ...
    if (action === 'save') {
      setFloatingText('Nice! You chose future you.')
    }
  ...
  return (
    <div className={`w-full max-w-5xl glass-panel p-5 md:p-7 ${financialHealth === 'danger' ? 'ring-2 ring-rose-300' : financialHealth === 'warning' ? 'ring-2 ring-amber-300' : 'ring-2 ring-emerald-300'}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="stat-card"><div className="stat-label">💰 Money</div><div className="stat-value">₹{money}</div></div>
        <div className="stat-card"><div className="stat-label">😊 Happiness</div><div className="stat-value">{happiness}</div></div>
        <div className="stat-card"><div className="stat-label">🏦 Savings</div><div className="stat-value">₹{savings}</div></div>
        <div className="stat-card"><div className="stat-label">⭐ Credit</div><div className="stat-value">{credit}</div></div>
      </div>
