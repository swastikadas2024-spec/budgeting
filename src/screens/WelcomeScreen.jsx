...
export default function WelcomeScreen({ hasSavedGame, onStart, onContinue }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-lg md:text-2xl mb-5 text-slate-700 font-semibold">30 Days. Real Choices. Build Your Money Superpowers.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        <div>
          <label className="block text-lg md:text-xl font-bold text-slate-700 mb-2">Choose Character</label>
          <select className="p-3 rounded-xl w-full text-lg md:text-xl border-2 border-orange-200" value={char} onChange={(e) => setChar(e.target.value)}>
            <option>Gamer</option>
        ...
        <div>
          <label className="block text-lg md:text-xl font-bold text-slate-700 mb-2">Select Difficulty</label>
          <select className="p-3 rounded-xl w-full text-lg md:text-xl border-2 border-orange-200" value={difficulty} onChange={(e) => setDifficulty(e.target.value)}>
            {difficulties.map((d) => (
      ...
      <div className="rounded-2xl bg-gradient-to-r from-orange-50 to-amber-50 p-4 mb-5 border-2 border-orange-200">
        <h3 className="font-bold text-lg text-slate-700 mb-2">Difficulty Modes:</h3>
        {difficulties.map((d) => (
          <p key={d.name} className="text-base text-slate-700 mb-1">
      ...
      <div className="flex gap-3">
        <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} className="flex-1 pulse-btn text-white py-3 rounded-xl text-lg md:text-xl font-extrabold" onClick={() => onStart({ type: char, difficulty })}>Start Game</motion.button>
